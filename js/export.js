// ── CSV Export ───────────────────────────────────────────────────────────────
// Opens a dialog letting the user choose which columns to export and in what
// order (drag-to-reorder). Insertion order of db.strings is preserved.
...
function openExportDialog() {
  ...
  // exportCols is stored on window so drag handlers can mutate it between
  // re-renders without needing a closure.
  window._exportCols = [
...
function _renderExportDialog() {
  ...
  const itemsHtml = cols.map((c, i) => {
    ...
    return `
      <div class="col-picker-item" draggable="true" data-idx="${i}"
        ...
        <input class="col-picker-check" type="checkbox"
          ${c.enabled ? 'checked' : ''} ${isRequired ? 'disabled' : ''}
  ...
  showModal(`
    <div class="modal" onclick="event.stopPropagation()" style="width:460px">
      ...
      <div class="modal-sub">Choose and reorder columns. KEY is always included.</div>
      <div class="picker-actions-row">
        ...
        <span style="margin-left:auto;font-family:var(--mono);font-size:11px;color:var(--text-muted)">
          ${enabledCount} of ${cols.length} columns
        </span>
      </div>
      ...
      <div class="modal-actions">
        ...
        <button class="btn btn-export" onclick="doExportCSV()" style="padding:7px 18px;font-size:12px">
          ↓ Download CSV
        </button>
      </div>
    </div>`);
...
function exportDrop(e, idx) {
  ...
  const [moved] = window._exportCols.splice(_dragSrcIdx, 1);
  window._exportCols.splice(idx, 0, moved);
  _dragSrcIdx = null;
  _renderExportDialog();
}
...
function exportToggleCol(idx, val) {
  ...
  const count   = window._exportCols.filter(c => c.enabled).length;
  const countEl = document.querySelector('.picker-actions-row span');
  if (countEl) countEl.textContent = `${count} of ${window._exportCols.length} columns`;
}
